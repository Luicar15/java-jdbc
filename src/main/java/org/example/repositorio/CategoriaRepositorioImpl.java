...
public class CategoriaRepositorioImpl implements Repositorio<Categoria> {
    ...
    public CategoriaRepositorioImpl(Connection coon) {
        this.coon = coon;
    }
    ...
    public Categoria porId(Long id) throws SQLException {
        ...
        try (PreparedStatement stmt = coon.prepareStatement("SELECT* FROM categorias as c WHERE c.id=?")){
            stmt.setLong(1, id);
    ...
    public void eliminar(Long id) throws SQLException {
        try (PreparedStatement stmt = coon.prepareStatement("DELETE FROM categorias WHERE id=?")){
            stmt.setLong(1, id);
            stmt.executeUpdate();
        }
